...
def graph():
    ...
    next_unix = last_unix + 86400  # 24 * 60 * 60

    for i in knnunipred:
        next_date = next_unix
        # next_unix += datetime.timedelta(days=1)
        next_unix = datetime.datetime.fromtimestamp(next_unix)
        next_unix += datetime.timedelta(days=1)
        next_unix = int(next_unix.timestamp())
    ...
    bars = ('KNN with uniform weights Accuracy', 'KNN with distinct weights Accuracy')
